...
interface DataSource {
  ...
  name: string
  description: string
  tier: 'standard' | 'connected' | 'professional' | 'agency'
...
const DATA_SOURCES: DataSource[] = [
  ...
  },
  {
    ...
    tier: 'professional',
    status: 'available',
    unlocksText: 'Unlocks third-party competitive and ranking data',
  },
  {
    ...
    tier: 'professional',
    status: 'coming_soon',
    unlocksText: 'Unlocks domain authority and backlink analysis',
  },
  {
    id: 'semrush',
    name: 'SEMrush / Ahrefs',
    description: 'Full keyword gap analysis, deep competitive benchmarking',
    tier: 'agency',
    status: 'coming_soon',
...
function ConnectModal({ source, onClose }: { source: DataSource; onClose: () => void }) {
  const isProfessional = source.tier === 'professional'
  ...
  return (
    <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50 p-4" onClick={onClose}>
      <div className="bg-white border border-[#E8E4DC] rounded-xl p-6 max-w-sm w-full shadow-lg" onClick={(e) => e.stopPropagation()}>
        ...
        <h3 className="text-[#1A1918] font-semibold text-base mb-1">{source.name}</h3>
        ...
        {source.id === 'ga4' || source.id === 'gsc' ? (
          ...
          <div className="bg-[#F4F2EF] border border-[#E8E4DC] rounded-lg p-4 mb-4 text-xs text-[#6B6560]">
            {source.status === 'coming_soon'
              ? 'This data source is part of the Professional tier and is coming soon.'
              : 'This data source is included in the Professional tier. Ask your account manager about upgrading.'}
          </div>
        ) : (
          <div className="bg-[#F4F2EF] border border-[#E8E4DC] rounded-lg p-4 mb-4 text-xs text-[#6B6560]">
            This integration is on the roadmap. Ask your account manager for availability.
          </div>
        )}

        <button onClick={onClose} className="w-full text-[#9C9690] hover:text-[#6B6560] text-xs mt-1 transition-colors">
          Close
        </button>
      </div>
    </div>
